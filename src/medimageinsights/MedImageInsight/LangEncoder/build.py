...
def build_tokenizer(config_encoder):
    ...
    if config_encoder['TOKENIZER'] == 'clip':
        ...
        # --- FIX START: Try-Except Block for Tokenizer ---
        try:
            # Try loading the requested tokenizer (local path)
            tokenizer = CLIPTokenizer.from_pretrained(pretrained_tokenizer, clean_up_tokenization_spaces=True)
        except TypeError:
            # Fallback if local config conflicts with new transformers library
            print(f"⚠️ Warning: Failed to load local tokenizer from {pretrained_tokenizer}. Falling back to default OpenAI CLIP.")
            tokenizer = CLIPTokenizer.from_pretrained("openai/clip-vit-base-patch32", clean_up_tokenization_spaces=True)
        
        # Safely set CLS token
