...
class MaskDecoder(nn.Module):
    ...
    def predict_masks(
        ...
        # FIX: Check if we need to repeat PE. 
        # If image_pe is [1, ...] and tokens is [B, ...], we repeat.
        # If image_pe is [B, ...] (already batched), we DO NOT repeat.
        if image_pe.shape[0] != tokens.shape[0]:
            pos_src = torch.repeat_interleave(image_pe, tokens.shape[0], dim=0)
        else:
            pos_src = image_pe # Already matches batch size
